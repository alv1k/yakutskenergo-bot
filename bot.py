...
async def process_district(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Примеры: Лермонтова 45, Лесная, переулок Сединский"
    )
    return SET_STREET
...
user_cooldowns = {}
CHECK_COOLDOWN = 300 # 5 minutes in seconds
...
async def check_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_cooldowns:
        last_check = user_cooldowns[user_id]
        if now - last_check < CHECK_COOLDOWN:
            remaining = int(CHECK_COOLDOWN - (now - last_check))
            await update.message.reply_text(f"Пожалуйста, подождите {remaining} сек. перед следующей проверкой.")
            return
    ...
    await update.message.reply_text("Проверка завершена. Если работ не найдено, уведомление не придет.")
...
def parse_russian_date(date_str):
    ...
    try:
        ...
        if not match:
            ...
            if range_match:
                day, month_name = int(range_match.group(2)), range_match.group(3).lower()
            else: return None
        else:
...
async def check_updates(application, target_chat_id=None):
    ...
    for chat_id, user_district, street in users:
        ...
        house_match = re.search(r'\s+(\d+[\w/]*)$', norm_user_input)
        house_num = house_match.group(1) if house_match else None
        street_name = norm_user_input[:house_match.start()].strip() if house_match else norm_user_input
        core_name = re.sub(r'^(ул|пер|пр|ш|наб|пл|пр-д|туп|б-р|тр)\s+', '', street_name)

        for s in schedules:
            sched_date = parse_russian_date(s['date'])
            if sched_date and sched_date < today: continue
            if norm_user_district in normalize_district(s['district']):
                ...
                if core_name in norm_schedule_addr:
                    if house_num:
                        ...
                        if range_match:
                            try:
                                if house_num.isdigit() and int(range_match.group(1)) <= int(house_num) <= int(range_match.group(2)):
                                    matches.append(s)
                            except: pass
        ...
        if unique_matches:
            ...
            await application.bot.send_message(chat_id=chat_id, text="✅ Работ по вашему адресу не найдено.")
...
async def scheduler_task(application):
    while True:
        ...
        logging.info(f"Next update check in {(target - now).total_seconds()}s")
        await asyncio.sleep((target - now).total_seconds())
        await check_updates(application)

async def post_init(application):
    await application.bot.set_my_commands([
        ("start", "Главное меню"), ("status", "Мои настройки"), ("check", "Проверить сейчас")
    ])
